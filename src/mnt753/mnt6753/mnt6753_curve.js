...
// The 6th-degree extension field
export class FQ6 extends FQP {
    static get modulus_coeffs() {
        // The modulus of the polynomial in this representation of FQ6
        return [non_residue, 0, 0, 0, 0, 0] // Implied + [1]
    }

    mul(other){
        const non_residue = this.constructor.modulus_coeffs[0]
        const a1 = new FQ3([this.coeffs[0], this.coeffs[1], this.coeffs[2]])
        const b1 = new FQ3([this.coeffs[3], this.coeffs[4], this.coeffs[5]])

        const a2 = new FQ3([other.coeffs[0], other.coeffs[1], other.coeffs[2]])
        const b2 = new FQ3([other.coeffs[3], other.coeffs[4], other.coeffs[5]])
...
// The group of points on curve MNT6753 over FQ6
export class MNT6753CurvePointFQ6 extends MNT6753CurvePoint {

    static get a() {
        return new FQ6([0, 0, MNT6753CurvePoint.a, 0, 0, 0])
    }

    static get b() {
        return new FQ6([MNT6753CurvePoint.b.mul(non_residue), 0, 0, 0, 0, 0])
    }

    static get w() {
        return new FQ6([0, 1, 0, 0, 0, 0])
    }
    ...
    static twist(pt) {
        ...
        // where w**6 = x
        const nx = new FQ6([xcoeffs[0], 0, xcoeffs[1], 0, 0, 0])
        const ny = new FQ6([ycoeffs[0], 0, ycoeffs[1], 0, 0, 0])
        // Divide x coord by w**2 and y coord by w**3
        return new MNT6753CurvePointFQ6(nx.mul(this.w.pow(2n)), ny.mul(this.w.pow(3n)))
    }
...
assert(MNT6753CurvePointFQ6.G.is_well_defined())

console.log('MNT6753CurvePointFQ6')